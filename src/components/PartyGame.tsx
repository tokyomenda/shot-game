...
export default function PartyGame() {
  ...
  function startGame() {
    ...
    setDeck(shuffleDeck()); setCardIndex(0); setTurn(0); setPhase("hidden");
  ...
  function finishTurn() {
    ...
    timer.current = setTimeout(() => {
      if (cardIndex + 1 >= deck.length) { setDeck(shuffleDeck(deck[cardIndex].id)); setCardIndex(0); }
  ...
  return (
    <main className={`app-shell screen-${screen}`}>
      <header className="topbar">
        {screen === "setup" ? <button className="icon-button" onClick={() => setScreen("home")} aria-label="Нүүр хуудас руу буцах">←</button> : <Brand />}
      ...
      {screen === "setup" && <section className="setup page-enter">
        ...
        <form onSubmit={event => { event.preventDefault(); startGame(); }}>
          <div className="section-label"><span>ТОГЛОГЧИД</span><span>{names.length} / 10</span></div>
          <div className="player-inputs">{names.map((name, index) => <div className="player-input" key={index}>
            ...
            <button type="button" className="remove-player" aria-label={`${index + 1}-р тоглогчийг хасах`} disabled={names.length <= 2} onClick={() => setNames(current => current.filter((_, i) => i !== index))}>×</button>
          </div>)}</div>
          {names.length < 10 && <button type="button" className="button add-button" onClick={() => setNames(current => [...current, ""])}><span>＋</span> Тоглогч нэмэх</button>}
      ...
      {screen === "game" && active && <section className="game page-enter">
        ...
        <div className="card-actions">{phase === "hidden" ? <div className="reveal-prompt"><span>↑</span><h3>КАРТАА НЭЭ</h3><p>Хөзөр дээр товшоод зориг гарга.</p></div> : <div className="revealed-actions page-enter"><button className="button primary" disabled={phase !== "revealed"} onClick={() => advance(false)}>БИЕЛҮҮЛСЭН <span>✓</span></button><button className="button penalty-button" disabled={phase !== "revealed"} onClick={() => advance(true)}>1 SHOT <span>🥃</span></button></div>}</div>
      ...
      {screen === "results" && <section className="results page-enter"><div className="result-icon">✦</div><div className="eyebrow">САЙХАН ҮДЭШ БАЙЛАА</div><h2>Хөзөр дууслаа.<br /><em>Дурсамж үлдлээ.</em></h2><p className="subtitle">{turn} карт нээж, {players.reduce((sum, player) => sum + player.shots, 0)} шийтгэл авлаа.</p>
        ...
        <button className="button primary" onClick={startGame}>ДАХИН ТОГЛОХ <span>↻</span></button><button className="button text-button" onClick={() => setScreen("setup")}>Тоглогчдоо өөрчлөх</button>
      </section>}
